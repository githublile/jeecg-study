package org.core.extend.swftools;
...
public class OpenOfficePDFConverter implements PDFConverter {
	...
	public void convert2PDF(String inputFile, String pdfFile, String extend) {
		...
		if (extend.equals("txt")) {
			...
			if (new File(odtFile).exists()) {
				org.core.util.LogUtil.info("odt文件已存在！");
				inputFile = odtFile;
			} else {
				try {
					...
					org.core.util.LogUtil.info("Odt文档不存在！");
					e.printStackTrace();
				}
			}
		...
		//pdfFile = pdfFile.replaceAll(" ", "").replaceAll("　", "");
		 org.core.util.LogUtil.info("进行文档转换转换:" + inputFile + " --> " + pdfFile);
		...
		try {
			...
			org.core.util.LogUtil.info(e.getMessage());
		}
		
		stopService();
	    org.core.util.LogUtil.info("进行文档转换转换---- 结束----");
	}
	...
	public static void startService() {
		...
		try {
			...
			org.core.util.LogUtil.info("office转换服务启动成功!");
		} catch (Exception ce) {
			org.core.util.LogUtil.info("office转换服务启动失败!详细信息:" + ce);
		}
	}

	public static void stopService() {
		org.core.util.LogUtil.info("关闭office转换服务....");
		if (officeManager != null) {
			officeManager.stop();
		}
		org.core.util.LogUtil.info("关闭office转换成功!");
	}
}
